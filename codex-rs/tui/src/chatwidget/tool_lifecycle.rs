...
impl ChatWidget {
    ...
    pub(super) fn on_view_image_tool_call(&mut self, path: LegacyAppPathString) {
        ...
        self.push_tool_activity(
            /*id*/ None,
            history_cell::new_view_image_tool_call(path, &self.config.cwd),
        );
    }
    ...
    pub(super) fn on_web_search_begin(&mut self, call_id: String) {
        ...
        self.push_tool_activity(
            Some(call_id.clone()),
            history_cell::new_active_web_search_call(
                call_id,
                String::new(),
    ...
    pub(super) fn on_web_search_end(
        ...
        self.flush_answer_stream_with_separator();
        if let Some(cell) =
            self.active_tool_cell_mut::<WebSearchCell>(|cell| cell.call_id() == call_id)
        {
            ...
            self.upsert_tool_activity(
                call_id.clone(),
                history_cell::new_web_search_call(call_id, query, action),
            );
        }
    ...
    pub(super) fn on_collab_agent_tool_call(&mut self, item: ThreadItem) {
        ...
        if let Some(cell) = multi_agents::tool_call_history_cell(
            ...
            self.flush_answer_stream_with_separator();
            self.upsert_tool_activity(id.clone(), cell);
        }
    }
    ...
    pub(super) fn on_sub_agent_activity(&mut self, item: ThreadItem) {
        if let Some(cell) = multi_agents::sub_agent_activity_history_cell(&item) {
            self.flush_answer_stream_with_separator();
            self.push_tool_activity(/*id*/ None, cell);
        }
    }
    ...
    pub(crate) fn handle_file_change_completed_now(&mut self, item: ThreadItem) {
        ...
        if matches!(status, codex_app_server_protocol::PatchApplyStatus::Failed) {
            self.push_tool_activity(
                /*id*/ None,
                history_cell::new_patch_apply_failure(String::new()),
            );
        }
    ...
    pub(crate) fn handle_mcp_tool_call_started_now(&mut self, item: ThreadItem) {
        ...
        self.push_tool_activity(
            Some(id.clone()),
            history_cell::new_active_mcp_tool_call(
                id,
                McpInvocation {
                    server,
                    tool,
    ...
    pub(crate) fn handle_mcp_tool_call_completed_now(&mut self, item: ThreadItem) {
        ...
        let extra_cell = if let Some(cell) =
            self.active_tool_cell_mut::<McpToolCallCell>(|cell| cell.call_id() == id)
        {
            ...
            let extra_cell = cell.complete(duration, result);
            self.push_tool_activity(Some(id), cell);
            extra_cell
        };

        if let Some(extra) = extra_cell {
            self.push_boxed_tool_activity(/*id*/ None, extra);
        }
